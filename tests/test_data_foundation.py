...
def test_api_football_results_use_date_batches_not_per_league():
    ...
    assert all(path != "leagues" for path, _ in provider.calls)
...
def test_api_football_standings_use_configured_season_without_league_discovery():
    provider = _FakeAPIFootballProvider()
    provider.fetch_standings()
    assert all(path != "leagues" for path, _ in provider.calls)
    standings_calls = [(path, params) for path, params in provider.calls if path == "standings"]
    assert len(standings_calls) == len(load_enabled_leagues())
    assert all("league" in params and "season" in params for _, params in standings_calls)
