...
Uses the leakage-safe feature engineering and CatBoost model. Historical model
features come from the audited xgabora dataset when its generated feature file
is present; the old football-data local loader remains only as an explicit
legacy fallback. Optional calibration is applied when available.
"""
...
XGABORA_FEATURE_PATH = Path(os.getenv("FOVRA_XGABORA_FEATURE_PATH", "data/processed/xgabora_merged_dataset.csv"))
...
class CanonicalPredictionService:
    def __init__(self, model_path: str | None = None, calibration_path: str | None = None, store: NeonStore | None = None):
        ...
        self.feature_schema_version = os.getenv("FOOTBALL_FEATURE_SCHEMA_VERSION", "leakage-safe-v1-xgabora")
    ...
    def _load_feature_data() -> pd.DataFrame:
        if XGABORA_FEATURE_PATH.exists():
            frame = pd.read_csv(XGABORA_FEATURE_PATH, low_memory=False)
            frame["Date"] = pd.to_datetime(frame["Date"], errors="coerce")
            required = {"Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG", "FTR"}
            missing = required - set(frame.columns)
            if missing:
                raise RuntimeError(f"xgabora feature dataset missing required columns: {sorted(missing)}")
            return frame.sort_values("Date").reset_index(drop=True)
        raise RuntimeError(
            "xgabora feature dataset is missing. Run scripts/build_xgabora_features.py "
            "before generating predictions; legacy Football-Data data is not used implicitly."
        )
