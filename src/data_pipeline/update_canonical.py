...
def run(*, offline: bool = False, sqlite_local: bool = False, db_path: str = "data/processed/fovra_data.sqlite3", source: str = "api-football", mode: str = "fixtures") -> dict:
    ...
    logger.info("FOVRA: fetched %s matches, %s leagues, %s teams", len(snapshot.matches), len(snapshot.leagues), len(snapshot.teams))
    ...
    try:
        upserted = store.upsert_snapshot(snapshot.leagues, snapshot.teams, snapshot.matches, snapshot.provider, snapshot.fetched_at)
        venue_updates = 0
        ...
        store.upsert("data_sources", [{"provider_key": snapshot.provider, "display_name": snapshot.provider, "last_attempt_at": now, "last_success_at": now, "last_data_at": snapshot.fetched_at, "last_success_rows": len(snapshot.matches), "last_error": None}], "provider_key")
        ...
        return {"provider": snapshot.provider, "fetched_at": snapshot.fetched_at, "records_seen": len(snapshot.matches), "records_upserted": upserted, "venue_updates": venue_updates, "api_football_requests": getattr(provider, "request_count", None), "prediction_results_resolved": resolved, "newest_match_at": newest, "storage": "neon-postgresql", "offline": offline}
    except Exception as exc:
        logger.exception("FOVRA: canonical ingestion failed")
        try:
            ...
            logger.exception("FOVRA: could not record ingestion failure metadata")
        raise
...
def main() -> None:
    ...
    parser.add_argument("--sqlite-local", action="store_true", help="Use SQLite only for local testing; never use in production")
    ...
    result = run(offline=args.offline, sqlite_local=args.sqlite_local, db_path=args.db, source=args.source, mode=args.mode)
