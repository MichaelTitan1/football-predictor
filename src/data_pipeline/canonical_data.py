...
class MatchRecord:
    ...
    def match_key(self) -> str:
        raw = "|".join([self.provider, self.league_key, self.season or "", self.kickoff_utc, self.home_team, self.away_team])
...
def _validate_match(match: MatchRecord) -> None:
    if not match.league_key or not match.home_team or not match.away_team or match.home_team == match.away_team:
    ...
    if any(score is not None and score < 0 for score in (match.home_score, match.away_score)):
...
def upsert_records(conn: sqlite3.Connection, leagues: Iterable[LeagueRecord], teams: Iterable[TeamRecord], matches: Iterable[MatchRecord], provider: str) -> int:
    ...
    with conn:
        for league in leagues:
            conn.execute("insert into leagues values(?,?,?,?) on conflict(league_key) do update set name=excluded.name,country=excluded.country,updated_at=excluded.updated_at", (league.key, league.name, league.country, now))
        for team in teams:
            conn.execute("insert into teams values(?,?,?,?) on conflict(league_key,team_key) do update set name=excluded.name,updated_at=excluded.updated_at", (team.key, team.league_key, team.name, now))
        for match in matches:
            _validate_match(match)
            conn.execute("""insert into matches(match_key,provider,source_id,league_key,season,kickoff_utc,home_team_key,away_team_key,status,home_score,away_score,first_seen_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?) on conflict(match_key) do update set source_id=coalesce(excluded.source_id,matches.source_id),status=case when excluded.status='finished' then 'finished' else excluded.status end,home_score=coalesce(excluded.home_score,matches.home_score),away_score=coalesce(excluded.away_score,matches.away_score),updated_at=excluded.updated_at""", (match.match_key,match.provider,match.source_id,match.league_key,match.season,match.kickoff_utc,match.home_team,match.away_team,match.status,match.home_score,match.away_score,now,now))
            count += 1
...
def record_source_error(conn: sqlite3.Connection, provider: str, error: str, records_seen: int = 0) -> None:
    now = utc_now()
    with conn:
        conn.execute("insert into source_updates(provider,last_success_at,last_attempt_at,records_seen,records_upserted,error) values(?,null,?,?,0,?) on conflict(provider) do update set last_attempt_at=excluded.last_attempt_at,records_seen=excluded.records_seen,error=excluded.error", (provider,now,records_seen,error[:1000]))
