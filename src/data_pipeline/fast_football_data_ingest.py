...
The source has two different layouts and the pipeline treats them differently:
22 main divisions are season-by-season files; 16 extra divisions are one
all-seasons-in-one CSV per league.  The extra feeds are filtered to 2010+
locally, because they contain their own historical dates and are not a
per-season URL family.
...
and is deliberately independent from the slower legacy row-by-row upsert path.
"""
...
SINGLE_LEAGUES = tuple(
    key for key, info in LEAGUE_CONFIG.items() if info.get("source_type") == "single"
)
SEASON_LEAGUES = tuple(
    key for key, info in LEAGUE_CONFIG.items() if info.get("source_type") != "single"
)
...
def _fetch_single_feed(code: str) -> bytes:
    ...
    for attempt in range(1, SINGLE_RETRIES + 1):
        try:
            response = session.get(url, timeout=SINGLE_TIMEOUT)
            if response.status_code == 404:
                raise RuntimeError(f"Football-Data returned HTTP 404 for {url}")
            response.raise_for_status()
            if not response.content:
                raise RuntimeError(f"Football-Data returned an empty CSV for {code}")
            return response.content
        except Exception as exc:
            last_error = exc
            if attempt < SINGLE_RETRIES:
                delay = min(2 ** (attempt - 1), 16)
                logger.warning("%s feed attempt %d/%d failed: %s; retrying in %ss", code, attempt, SINGLE_RETRIES, exc, delay)
                time.sleep(delay)
    raise RuntimeError(f"Unable to download Football-Data extra league {code} after {SINGLE_RETRIES} attempts: {last_error}")
...
def refresh_single_leagues() -> dict[str, Any]:
    ...
    for code in SINGLE_LEAGUES:
        url_key = f"{code}:new"
        try:
            payload = _fetch_single_feed(code)
            ...
            frame = frame.loc[parsed.dt.year.ge(SINGLE_START_YEAR).fillna(False)].copy()
            ...
            logger.info("Refreshed %s all-seasons feed: %d rows kept (Date >= %d)", code, len(frame), SINGLE_START_YEAR)
        except Exception as exc:
            failed.append(code)
...
def _match_rows(matches: list[MatchRecord], provider: str, fetched_at: str) -> list[tuple[Any, ...]]:
    return [
        (
            ...
            m.kickoff_utc,
            m.status,
            f"{m.league_key}:{m.home_team}",
            f"{m.league_key}:{m.away_team}",
            m.home_score,
            m.away_score,
            provider,
            m.source_id or m.match_key,
            fetched_at,
...
def _bulk_upsert(store: NeonStore, snapshot: Any) -> int:
    ...
    league_rows = [
        (l.key, l.name, l.country, snapshot.provider, snapshot.fetched_at, now)
        for l in snapshot.leagues
    ]
    team_rows = [
        (f"{t.league_key}:{t.key}", t.league_key, t.name, snapshot.provider, snapshot.fetched_at, now)
    ...
    # COPY in bounded chunks keeps memory predictable while avoiding per-row SQL.
    for start in range(0, len(rows), MATCH_STAGE_BATCH):
        chunk = rows[start:start + MATCH_STAGE_BATCH]
        with conn.cursor() as cur:
            with cur.copy(
                "COPY fovra_match_stage (canonical_key,league_canonical_key,season,kickoff_at,status,home_team_canonical_key,away_team_canonical_key,home_score,away_score,source_provider,source_match_id,source_updated_at,updated_at) FROM STDIN"
            ) as copy:
                for row in chunk:
                    copy.write_row(row)
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            ON CONFLICT (canonical_key) DO UPDATE SET
                league_canonical_key=EXCLUDED.league_canonical_key,
        ...
        cur.execute(
            ...
            SELECT source_provider,'match',source_match_id,canonical_key,
                   jsonb_build_object(
                     'league_key',league_canonical_key,'season',season,'kickoff_utc',kickoff_at,
...
def run() -> dict[str, Any]:
    ...
    try:
        ...
        store.upsert(
            "provider_sources",
            [{"provider_key": snapshot.provider, "display_name": snapshot.provider, "source_type": "match-data", "updated_at": datetime.now(timezone.utc).isoformat()}],
            "provider_key",
        )
        store.upsert(
            "data_sources",
        ...
        store.record_ingestion_finish(
            ...
            newest_match_at=max((m.kickoff_utc for m in snapshot.matches), default=None),
        )
        return {
