...
def run() -> dict[str, Any]:
    ...
    try:
        ...
        store.upsert("data_sources", [{"provider_key": snapshot.provider,"display_name": snapshot.provider,"last_attempt_at": datetime.now(timezone.utc).isoformat(),"last_success_at": datetime.now(timezone.utc).isoformat(),"last_data_at": snapshot.fetched_at,"last_success_rows": len(snapshot.matches),"last_error": None,"freshness_policy": "Monday/Wednesday Football-Data refresh; incremental local cache"}], "provider_key")
        ...
        return {"provider": snapshot.provider,"records_seen": len(snapshot.matches),"records_upserted": upserted,"leagues_seen": len(actual),"season_refresh": season_result,"single_refresh": single_result,"predictions_resolved": resolved}
    except Exception as exc:
